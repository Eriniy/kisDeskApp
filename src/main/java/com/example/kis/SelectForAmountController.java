...
public class SelectForAmountController {
    ...
    void initialize() {
        ...
        btn_showSelect.setOnAction(event ->{
            if (selectPar_field.getText()!=null && isValidNumber(selectPar_field.getText())){
                createSelectContent();
            }
            else {
                ...
                alert.setContentText("Параметр выборки введен некорректно или" +
                        " не введен вообще!");
                alert.showAndWait();
            }
    ...
    private void createSelectContent() {
        ...
        ObservableList<Content> selectList =
                connDB.getSelectForAmount(selectPar_field.getText());
    ...
    private void setTooltip(){
        selectPar_field.setTooltip(new Tooltip("число, меньше которого " +
                "выведутся записи"));
    }
