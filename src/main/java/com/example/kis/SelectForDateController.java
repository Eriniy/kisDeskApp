...
public class SelectForDateController {
    ...
    @FXML
    private DatePicker selectPar_field;
    ...
    void initialize() {
        ...
        btn_showSelect.setOnAction(event ->{
            if (selectPar_field.getValue().toString()!=null){
                createSelectContent();
            }
    ...
    private void createSelectContent() {
        ...
        ObservableList<Content> selectList =
                connDB.getSelectForDate(selectPar_field.getValue().toString());
    ...
    private void setTooltip(){
        selectPar_field.setTooltip(new Tooltip("дата, с которой будут " +
                "показаны записи"));
    }
